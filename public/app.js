...
const state={screen:'home',category:'الكل',products:[],loading:true,error:'',cart:[],selected:null,qty:1,delivery:{province:'',city:'',name:'',address:'',landmark:'',phone:'',backup:''},payment:null,orderNo:'',session:null,user:null,qrConfirmed:false,paymentNote:'',carousel:0,authMode:'login'};
...
function openQR(){const modal=document.getElementById('modal');if(!modal)return toast('تعذر فتح نافذة الدفع');modal.innerHTML=`<div class="modal-card payment-modal"><button type="button" class="close" data-close aria-label="إغلاق">×</button><div class="mini-logo large"><strong>آدم</strong><small>ADAM LIBRARY</small></div><h2>الدفع الإلكتروني عبر QR</h2><p>افتح تطبيق مصرفك وامسح الكود التالي، ثم أدخل المبلغ الظاهر أدناه.</p><div class="qr"><img src="/assets/payment-qr.jpeg" alt="رمز الدفع QR"></div><div class="amount-card"><span>المبلغ المطلوب</span><strong>${money(total())}</strong><button type="button" data-copy-amount>نسخ المبلغ</button></div><label class="label">رقم العملية / ملاحظة (اختياري)<input class="input" data-field="paymentNote" value="${esc(state.paymentNote||'')}" placeholder="أدخل رقم العملية أو أي ملاحظة"></label><button type="button" class="gold-btn" data-qr-done="1">تم الدفع / متابعة ✓</button></div>`;modal.classList.remove('hidden')}
...
async function handleAppClick(e){
  ...
  try{
    ...
    if(Object.prototype.hasOwnProperty.call(d,'qrDone')){state.paymentNote=document.querySelector('[data-field="paymentNote"]')?.value.trim()||state.paymentNote||'';state.qrConfirmed=true;closeModal();return confirmOrder(t)}
